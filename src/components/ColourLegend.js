...
export default function ColourLegend({ variableConfig, selectedVariable }) {
    ...
    return (
        <Paper elevation={4} sx={{ position: 'absolute', bottom: 40, left: 16, p: 1.5, display: 'flex', flexDirection: 'column',alignItems: 'center', gap: 1, backgroundColor: 'rgba(30, 41, 59, 0.9)' }}>
            <Box>
                <Typography>{displayConfig.units}</Typography>
            </Box>
            <Box sx={{display: 'flex'}}>
                <Box sx={{ background: gradient, width: '20px', height: '40vh', borderRadius: '4px', marginRight: '10px' }}></Box>
